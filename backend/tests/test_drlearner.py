"""Tests for the revised Phase 3 DRLearner conversion slice."""
...
def prepared_manifest(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """Create a small prepared dataset manifest for DRLearner tests."""

    tmp_path = tmp_path_factory.mktemp("drlearner")
...
def test_train_drlearner_conversion_model_saves_artifacts_and_scores(
    prepared_manifest: Path, tmp_path: Path
) -> None:
    """Training should write the model bundle, metric report, and scored holdout splits."""

    report = train_drlearner_conversion_model(
        prepared_manifest_path=prepared_manifest,
        model_artifact_path=tmp_path / "drlearner.joblib",
        metrics_report_path=tmp_path / "drlearner_report.json",
...
def test_drlearner_scored_outputs_include_prediction_column(
    prepared_manifest: Path, tmp_path: Path
) -> None:
    """The saved scored split files should contain the DRLearner prediction column."""
    ...
    for split_evaluation in report.split_evaluations:
        ...
        assert DRLEARNER_CONVERSION_SCORE_COLUMN in scored_frame.columns
        assert scored_frame[DRLEARNER_CONVERSION_SCORE_COLUMN].notnull().all()
        assert split_evaluation.row_count == len(scored_frame)


def test_saved_drlearner_bundle_can_be_loaded(prepared_manifest: Path, tmp_path: Path) -> None:
    """The persisted DRLearner bundle should be reloadable from disk."""
    ...
    bundle = load_trained_drlearner_bundle(model_artifact_path)

    assert bundle["model_name"] == "drlearner_conversion"
    assert bundle["score_column"] == DRLEARNER_CONVERSION_SCORE_COLUMN
