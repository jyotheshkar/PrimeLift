"""Script wrapper for the second revised Phase 4 model-based uplift slice."""
...
def _build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Generate model-based cohort rollups using the selected Phase 3 champion."
    )
    parser.add_argument(
        "--comparison-report",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Path to the raw dataset used to enrich scored outputs with business dimensions.",
    )
    parser.add_argument(
        "--outcome",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Holdout split to roll up by business dimension.",
    )
    parser.add_argument(
        "--output-report",
        type=Path,
        default=DEFAULT_PHASE4_CONVERSION_ROLLUP_REPORT_PATH,
        help="Path where the rollup report JSON will be written.",
    )
    parser.add_argument(
        "--rollup-table",
        type=Path,
        default=DEFAULT_PHASE4_CONVERSION_ROLLUP_TABLE_PATH,
        help="Path where the flattened rollup table CSV will be written.",
    )
    parser.add_argument(
        "--enriched-scored-view",
        type=Path,
        default=DEFAULT_PHASE4_CONVERSION_ENRICHED_SCORED_VIEW_PATH,
        help="Path where the enriched scored view CSV will be written.",
    )
    return parser
...
def main() -> None:
    """Generate the second revised Phase 4 model-based uplift report."""
    ...
    report = generate_model_based_group_rollup_report(
        ...
        rollup_table_path=args.rollup_table,
        enriched_scored_view_path=args.enriched_scored_view,
    )
