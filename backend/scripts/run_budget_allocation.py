"""Script wrapper for the revised Phase 5 budget-allocation slice."""
...
def _build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Generate the revised Phase 5 model-driven budget allocation report."
    )
    parser.add_argument(
        "--conversion-rollup-report",
        type=Path,
        default=DEFAULT_PHASE4_CONVERSION_ROLLUP_REPORT_PATH,
        help="Path to the Phase 4 conversion rollup report JSON.",
    )
    parser.add_argument(
        "--revenue-metrics-report",
        type=Path,
        default=DEFAULT_DRLEARNER_REVENUE_METRICS_PATH,
        help="Path to the Phase 3 revenue model training report JSON.",
    )
    parser.add_argument(
        "--output-report",
        type=Path,
        default=DEFAULT_PHASE5_BUDGET_ALLOCATION_REPORT_PATH,
        help="Path where the Phase 5 budget allocation report JSON will be written.",
    )
    parser.add_argument(
        "--budget-table-path",
        type=Path,
        default=DEFAULT_PHASE5_SEGMENT_BUDGET_TABLE_PATH,
        help="Path where the segment budget allocation CSV will be written.",
    )
    parser.add_argument(
        "--total-budget",
        type=float,
        default=100_000.0,
        help="Total budget pool to allocate across prioritized segments.",
    )
    return parser
...
def _ensure_phase4_rollup_exists(conversion_rollup_report_path: Path) -> None:
    """Create prerequisite model artifacts if they are missing."""
    ...
    if not conversion_rollup_report_path.exists():
        generate_model_based_group_rollup_report(
            comparison_report_path=DEFAULT_PHASE3_MODEL_COMPARISON_REPORT_PATH,
            outcome_column="conversion",
            split_name="test",
            output_report_path=conversion_rollup_report_path,
            enriched_scored_view_path=DEFAULT_PHASE4_CONVERSION_ENRICHED_SCORED_VIEW_PATH,
        )
    if not DEFAULT_PHASE4_CONVERSION_VALIDATION_SUMMARY_PATH.exists():
        generate_phase4_validation_summary(
            ...
            rollup_report_path=conversion_rollup_report_path,
            output_report_path=DEFAULT_PHASE4_CONVERSION_VALIDATION_SUMMARY_PATH,
        )


def main() -> None:
    """Generate the revised Phase 5 budget allocation report."""

    parser = _build_argument_parser()
    args = parser.parse_args()
    _ensure_phase4_rollup_exists(args.conversion_rollup_report)

    report = generate_segment_budget_allocation(
        conversion_rollup_report_path=args.conversion_rollup_report,
        revenue_metrics_report_path=args.revenue_metrics_report,
        output_report_path=args.output_report,
        budget_table_path=args.budget_table_path,
